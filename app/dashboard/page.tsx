...
import { mockStickers, TOTAL_STICKERS, countryFlags } from "@/lib/mock-data";
import { ProgressBar } from "@/components/progress-bar";
import { SummaryCards } from "@/components/summary-cards";
import { Navbar } from "@/components/navbar";
import { cn } from "@/lib/utils";

// Group stickers by country for quick stats
function useAlbumStats(stickers: typeof mockStickers) {
  return useMemo(() => {
    ...
    const progress = (owned / TOTAL_STICKERS) * 100;

    // Per-country completion
    const byCountry = stickers.reduce<Record<string, { total: number; owned: number; code: string }>>((acc, s) => {
    ...
    const countryStats = Object.entries(byCountry)
      .map(([name, v]) => ({ name, ...v, pct: Math.round((v.owned / v.total) * 100) }))
      .sort((a, b) => b.pct - a.pct);

    return { owned, missing, duplicates, progress, countryStats };
  }, [stickers]);
}

export default function DashboardPage() {
  const [stickers] = useState(mockStickers);
  const { owned, missing, duplicates, progress, countryStats } = useAlbumStats(stickers);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-2xl px-4 py-6 space-y-6">
          ...
          <div className="rounded-3xl bg-primary p-5 text-primary-foreground shadow-md relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="mt-4 rounded-2xl bg-white/15 p-4">
                <ProgressBar value={progress} total={TOTAL_STICKERS} owned={owned} />
              </div>
            </div>
          </div>
        </section>
        ...
        <section aria-labelledby="summary-heading">
          <h2 id="summary-heading" className="sr-only">Resumen del Álbum</h2>
          <SummaryCards
        ...
        <section aria-labelledby="actions-heading">
          <h2 id="actions-heading" className="font-display text-base font-bold uppercase tracking-wider text-foreground mb-3">
            Acciones Rápidas
          </h2>
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">{duplicates} figuritas disponibles para intercambio</p>
              </div>
        ...
        <section aria-labelledby="countries-heading">
          <h2 id="countries-heading" className="font-display text-base font-bold uppercase tracking-wider text-foreground mb-3">
            Países
          </h2>
          <div className="rounded-2xl border border-border bg-card shadow-sm divide-y divide-border overflow-hidden">
            {countryStats.map(({ name, owned: cOwned, total, pct, code }) => (
              <div key={name} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors">
                <span className="text-xl leading-none" aria-hidden="true">{countryFlags[code] ?? "🏳️"}</span>
